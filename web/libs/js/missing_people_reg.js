...
function uploadPhotoAndStore() {
    ...
    xhr.open("POST", "PhotoUploadController", true);
    xhr.send(formData);
}
...
function doRegistrate(){
    // 表单对象
    var name = ID("mp-name");
    var birthDate = ID("mp-birthdate");
    var height = ID("mp-height");
    var lostDate = ID("mp-lostdate");
    var detailAddress = ID("mp-missingplace-d");
    var description = ID("mp-description");
    var extraDescription = ID("mp-extra-description");
    var lng = ID("longitude");
    var lat = ID("latitude");

    // 在此处将拖动后位置传给表单
    //lng.value = locationCoordinate[0];
    //lat.value = locationCoordinate[1];
    console.log(lng.value);

    if (name.value==""||birthDate.value==""||height.value==""||lostDate.value==""||
        detailAddress.value==""||description.value==""||extraDescription.value==""||
        lng.value==""||lat.value=="") {
        window.alert("请填写完整的失踪人员信息！");
        return;
    }
    if (photoData!="成功检测到人脸！") {
        window.alert("请上传含有单个人脸的图像文件！");
        return;
    }
    if (photoState!="照片上传成功！") {
        window.alert("照片上传失败请重亲上传！");
        return;
    }
    $.ajax({
        cache: false,
        type: "POST",
        url:"http://localhost:8080/pages/MissingPeopleRegistrationController", //把表单数据发送到Controller
...
$(document).ready(function () {
    $("#mp-name").blur(function () {
        var  x = document.getElementById("mp-name").value;
        var  reg=/[^\x00-\x80]/;
        if(x==null||x==""||!reg.test(x)){
            ...
            document.getElementById("right-name").style.display="block";
        }
    });
    $("#mp-name").focus(function () {
        document.getElementById("right-name").style.display="none";
        document.getElementById("wrong-name").style.display="none";})
    $("#mp-height").blur(function () {
        var  x = document.getElementById("mp-height").value;
        var  reg=/[0-9]+/;
        if(x==null||x==""||!reg.test(x)){
            document.getElementById("right-height").style.display="none";
            document.getElementById("wrong-height").style.display="block";
        }
        else {
            document.getElementById("wrong-height").style.display="none";
            document.getElementById("right-height").style.display="block";
        }
    })
    $("#mp-height").focus(function () {
        document.getElementById("right-height").style.display="none";
        document.getElementById("wrong-height").style.display="none";})
    ...
    $("#mp-missingplace-d").blur(function () {
        var  x = document.getElementById("mp-missingplace-d").value;
        var  reg=/[^\x00-\x80]/;
        if(x==null||x==""||!reg.test(x)){
            document.getElementById("right-missingplace-d").style.display="none";
            document.getElementById("wrong-missingplace-d").style.display="block";
        }
        else {
            document.getElementById("wrong-missingplace-d").style.display="none";
            document.getElementById("right-missingplace-d").style.display="block";
        }
        submitLocation();
    })

    $("#mp-missingplace-d").focus(function () {
        document.getElementById("wrong-missingplace-d").style.display="none";
        document.getElementById("right-missingplace-d").style.display="none";})

    $("#mp-description").blur(function () {
        var  x = document.getElementById("mp-description").value;
        var  reg=/[^\x00-\x80]/;
        if(x==null||x==""||!reg.test(x)){
            document.getElementById("right-description").style.display="none";
            document.getElementById("wrong-description").style.display="block";
        }
        else {
            document.getElementById("wrong-description").style.display="none";
            document.getElementById("right-description").style.display="block";
        }
    })
    $("#mp-description").focus(function () {
        document.getElementById("wrong-description").style.display="none";
        document.getElementById("right-description").style.display="none";})

    $("#mp-extra-description").blur(function () {
        var  x = document.getElementById("mp-extra-description").value;
        var  reg=/[^\x00-\x80]/;
        if(x==null||x==""||!reg.test(x)){
            document.getElementById("right-extra-description").style.display="none";
            document.getElementById("wrong-extra-description").style.display="block";
        }
        else {
            document.getElementById("wrong-extra-description").style.display="none";
            document.getElementById("right-extra-description").style.display="block";
        }
    })
    $("#mp-extra-description").focus(function () {
        document.getElementById("wrong-extra-description").style.display="none";
        document.getElementById("right-extra-description").style.display="none";})

    $("#mp-birthdate").blur(function () {
        var  x = document.getElementById("mp-birthdate").value;
        if(x==null||x==""){
            document.getElementById("right-birthdate").style.display="none";
            document.getElementById("wrong-birthdate").style.display="block";
        }
        else {
            document.getElementById("wrong-birthdate").style.display="none";
            document.getElementById("right-birthdate").style.display="block";
        }
    })
    $("#mp-birthdate").focus(function () {
        document.getElementById("wrong-birthdate").style.display="none";
        document.getElementById("right-birthdate").style.display="none";})

    $("#mp-lostdate").blur(function () {
        var  x = document.getElementById("mp-lostdate").value;
        if(x==null||x==""){
            document.getElementById("right-lostdate").style.display="none";
            document.getElementById("wrong-lostdate").style.display="block";
        }
        else {
            document.getElementById("wrong-lostdate").style.display="none";
            document.getElementById("right-lostdate").style.display="block";
        }
    })
    $("#mp-lostdate").focus(function () {
        document.getElementById("wrong-lostdate").style.display="none";
        document.getElementById("right-lostdate").style.display="none";})
...
function submitLocation() {
    ...
    var desc = document.getElementById("mp-missingplace-d").value;
    getLocation(province+"省"+city+"市"+desc);
}
    ...
    AMap.service('AMap.Geocoder',function(){//回调函数
        ...
        geocoder.getLocation(address, function(status, result) {
            if (status === 'complete' && result.info === 'OK') {
                ...
                console.log(ID("longitude").value);
                //地图视图的初始参数
